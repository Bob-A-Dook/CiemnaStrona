...
* [Instalowanie Pythona](#instalowanie-pythona)
  * [Na Windowsie](#na-windowsie)
  * [Na Linuksie](#na-linuksie)
* [Korzystanie ze skryptów](#korzystanie-ze-skryptów)
  * [Na Windowsie](#na-windowsie)
* [Pytania i&nbsp;odpowiedzi](#pytania-iodpowiedzi)
  * [A&nbsp;jeśli nie działa?](#ajeśli-nie-działa)
  * [Czy potrzeba czegoś poza Pythonem?](#czy-potrzeba-czegoś-poza-pythonem)
...
Oczywiście może nas najść ochota, żeby zamiast domyślnej wersji skorzystać również z&nbsp;najnowszej. Tu już jest nieco więcej kroków, więc pozwolę sobie podlinkować [fajny poradnik](https://askaholic.io/installing-python-from-source/) po angielsku. Więcej szczegółów na jego temat w&nbsp;rozwijanej zakładce.

{% include details.html summary="Kilka uwag do podlinkowanego poradnika" %}

* Plik z&nbsp;oficjalnej strony Pythona nie będzie miał formatu `exe`, tylko `tar.xz`.
* Trzeba otworzyć konsolę w&nbsp;tym samym folderze co ten plik.
  
  W&nbsp;tym celu otwieramy odpowiednik eksploratora plików na Mincie i&nbsp;przechodzimy do odpowiedniego folderu.  
  Klikamy prawym przyciskiem myszy gdzieś poza ikonami plików i&nbsp;wybieramy opcję `Otwórz w terminalu`. Można wybrać tę opcję również przez menu `Plik` w&nbsp;górnym lewym rogu.
  
* Wszystkie rzeczy z&nbsp;poradnika pokazane na czarnym tle to komendy. Wpisujemy je w&nbsp;naszej konsoli, bez dolarka na początku.
* Jeśli ufamy w&nbsp;ciemno wersji ze strony Pythona, to możemy pominąć pierwsze punkty poradnika i&nbsp;przeskoczyć od razu do ostatnich dwóch komend z&nbsp;punktu&nbsp;1 (od `tar`).
* Do systemu Linux Mint odnoszą się polecenia spod nagłówka *Ubuntu* (bo jest na nim oparty).
