...
<p style="display:none;"><img src="ciemna-strona-swieta.jpg" title="Przerobiona kartka świąteczna ze strony Miłakartka.pl" alt="Kartka świąteczna w&nbsp;pastelowych kolorach, pokazująca Mikołaja stojącego obok choinki. U&nbsp;góry widać napis 'Wesołych Świąt', a&nbsp;dolnym rogu 'Źródło oryginału: Miłakartka.pl'. Postać w&nbsp;stroju Mikołaja zamiast twarzy ma logo tego bloga. Również głowa bałwanka, zdobiąca napis, ma oczy skopiowane od emotki Ciemnej Strony."/></p>
...
W przypadku przeglądarki Chromium na Linuksie pamięć podręczna znajduje się w&nbsp;folderze `.cache/chromium/Default/Cache`.  
W przypadku Chrome'a na Windowsie -- w&nbsp;`AppData\Local\Google\Chrome\User Data\Default\Cache` (albo, ogólnie, w podfolderze `Appdata\Local`). 
...
To tak zwane **pliki binarne**. Nie mają żadnego rozszerzenia i&nbsp;są swego rodzaju pojemnikami na inne pliki, zwykle już o&nbsp;znanych typach. W&nbsp;każdym z&nbsp;nich mamy komplet złożony z&nbsp;oryginelnego pliku, a&nbsp;także garść informacji uzupełniających (jak link do źródła, z&nbsp;którego go pobraliśmy, albo datę i&nbsp;godzinę pobrania).
...
Ciemna Strona jest statyczna, a&nbsp;do każdego wpisu prowadzi inny link.  
...
Tym niemniej zawsze może się zdarzyć, że ktoś wyczyści historię, a&nbsp;pamięci podręcznej nie. Warto wiedzieć, że w&nbsp;takim przypadku *cache* może ją w&nbsp;znacznej części zastąpić jako kopalnia informacji.
...
A w&nbsp;cache'u? **Zapisały się tam (chyba) wszystkie obrazki, jakie mi się wyświetliły**. Zarówno na stronie, jak i&nbsp;na czacie. Miniaturki znajomych, z&nbsp;którymi coś pisaliśmy. Reklamy i&nbsp;ilustracje do wpisów, jakie akurat nam pokazywało na tablicy.
...
Ktoś z&nbsp;zewnątrz, zamiast zgadywać po awatarach, miałby nazwę jednego użytkownika na tacy.
...
Nawet gdybyśmy korzystali z publicznego komputera, gdzie pamięć podręczna przyjmuje obrazku różnych osób, ktoś mógłby łatwo porozdzielać zdjęcia z różnych kont, patrząc na godziny ich przesłania.  
...
Jeśli czytaliście moją najdłuższą serię, „Internetową inwigilację”, to pewnie jesteście już całkiem nieźle zaznajomieni z&nbsp;tym procederem :wink:
...
To unikalny ciąg znaków. Może wyglądać na przykład tak:
...
Ale powiedzmy, że jednak istnieje jakaś sprawiedliwość i&nbsp;w sytuacji takiej jak wyżej uznaliby naszą niewinność. Jasna sprawa, wiele pokrzywdzonych osób, świadkowie itp.
...
Osoba wrabiająca życzliwie zachęca nas do odwiedzenia strony X. Kiedy to robimy, za kulisami przeglądarka pobiera kompromitujący nas obrazek i&nbsp;zapisuje go na dysku.  
...
Zanim zaczniemy, jeszcze tylko drobna uwaga: jeśli usuwamy rzeczy z&nbsp;pamięci podręcznej, a&nbsp;potem odwiedzamy tę samą stronę, to wszystko załadujemy od nowa.  
...
{:.figure}
<img src="/assets/posts/caching/czyszczenie-historii.jpg" alt="Zrzut ekranu okna Firefoxa z&nbsp;zaznaczonymi wszystkimi opcjami czyszczenia danych przeglądania. Czerwoną ramką otoczono tekst „Pamięć podręczna”."/>
...
Od teraz nie będzie nam zapisywało do *cache'a* żadnych plików. Gdyby ktoś z&nbsp;zewnątrz chciał je dorwać, to musiałby mieć dostęp bezpośrednio do naszej pamięci RAM -- a&nbsp;gdyby miał, to znaczy że mamy dużo większe problemy niż jakieś rzeczy z&nbsp;*cache'a*!
...
<p>I działał! Ale pamiętajmy, że autorzy przeglądarek zawsze mogą coś zmienić w&nbsp;formacie plików binarnych z&nbsp;<em>cache’a</em>, przez co ubiją skrypt. W&nbsp;takim wypadku trzeba by go mocniej zmienić, co jest póki co poza moim zasięgiem. Żeby mieć większą pewność działania, najlepiej trzymajcie się Firefoxa.</p>
"%}
...
```python
# Można wyciągnąć wszystkie pliki ze znalezionych pamięci
extract_data()
...
Mamy też kilka funkcji do prostej analizy eksploracyjnej:

```python
# Zdobyć pliki png ze źródeł zawierających słowo 'facebook'
...
# Albo pokazać tylko adresy zawierające ciąg czterech dowolnych cyfr
# (korzystamy tu z&nbsp;wyrażeń regularnych)
